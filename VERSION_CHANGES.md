...
#### 1. UI/UXの改善
- **操作ボタンの追従表示**: ヘッダー領域（操作ボタンを含む）を `sticky` 配置に変更し、画面をスクロールしても常にボタンが表示されるように改善しました。
- **読み込み時の表示制御**: 初期データ読み込み中は操作ボタンや入力フォームを非表示にし、画面のちらつきや意図しない操作を防止するようにしました。
- **検索機能のユーザビリティ向上**:
  - **デバウンス処理**: 検索キーワード入力時に800msの入力を待機してから検索を実行するようにし、動作を軽快にしました。
  - **スクロール位置の保存**: 検索画面への切り替え・復帰時に、元の入力画面のスクロール位置を記憶・復元するようにしました。
...
#### 1. クライアントサイドキャッシュの改善
- **Cache API への移行**: 大学データのキャッシュを `localStorage` から **Cache API** に変更しました。
  - **シリアル番号によるキャッシュキー**: キャッシュキーにシリアル番号を含めることで、データ更新時の自動無効化を実現
  - **タイムスタンプ管理**: `X-Cache-Timestamp` ヘッダーで24時間の有効期限を管理
  - **容量制限の緩和**: localStorage の5-10MB制限を回避し、より大きなデータのキャッシュが可能に
- **JSON文字列パース処理追加**: サーバーからのレスポンスがJSON形式（ダブルクォート付き）で返される場合に対応

#### 2. 変更されたファイル
- `script.html`: キャッシュ関連関数をCache API対応に変更
  - `UNIVERSITY_DATA_CACHE`: `cacheName` と `cacheKey` 関数を追加
  - `tryLoadFromCache()`: Cache API使用に変更
  - `saveToCache()`: `Response` オブジェクトとして保存
  - `clearCache()`: `cache.delete()` で削除、非同期化
  - `decompressCachedData()`: JSON文字列パース処理を追加
...
#### 1. 大規模リファクタリング (サーバーサイド)
- **キャッシュアクセスの統一**: `getSheetDataApiWithCache` を標準とし、各機能でのデータ取得を統一しました。
- **シート定義の集中管理**: 散在していたシート名リテラルを `DATA_SHEETS` 定数オブジェクトに集約し、保守性を向上させました。
- **コード簡素化**: `getInitialData` や `getSettings` 内の重複コードを排除し、ヘルパー関数を活用する形に整理しました。

#### 2. 大規模リファクタリング (クライアントサイド)
- **DOMキャッシュの自動化**: `cacheDomElements` 関数により、IDを持つ要素を自動的にキャッシュし、`document.getElementById` の呼び出しを削減しました。
- **通信処理の責務分離**: `runGoogleScriptWithRetry` を、純粋なPromise通信関数 `runGoogleScript` と、リトライ制御ラッパーに分割し、再利用性を高めました。
- **コード整理**: デバッグログの削除やイベントハンドラの整理を行いました。

#### 3. 運用・保守性の向上
- 変数名や関数名をより実態に即したものに変更（例: `arrAllTeachers` → `allTeachersData`）。
- キャッシュすべき箇所（マスタ系）とすべきでない箇所（大容量データ・リアルタイムデータ）の区分けを明確化しました。
...
#### 1. 教員モードでの未入力生徒の可視化
- **登録数カラム追加**: `学籍データ`シートにF列「登録数」を追加し、各生徒の受験校登録件数を管理。
- **未入力者のハイライト**: 教員モードの生徒選択リストで、登録数が0の生徒を `--color-info`（青色）で表示。
- **保存後の自動更新**: 受験校データの保存時に登録数を自動更新し、生徒リストの表示色を即時反映（ページリロード不要）。

#### 2. サーバーサイド変更
- **`STUDENT_DATA.REG_COUNT`**: 生徒データ構造に登録数カラム（F列）を追加。
- **`updateStudentRegCount()`**: 生徒の登録数を学籍データシートに書き込むヘルパー関数を追加。
- **`sendExamData()`**: 保存完了時に登録数を更新する処理を追加。

#### 3. クライアントサイド変更
- **`createStudentList()`**: 登録数0の生徒に `is-empty` クラスを付与。
- **`onExamDataSent()`**: 保存後に `allStudentsData` の登録数を更新し、生徒リストを再描画。入力テーブルは非表示に。
- **`updateStudentRegCountClient()`**: クライアント側で登録数を更新するヘルパー関数を追加。

#### 4. スタイル変更
- **`option.is-empty`**: 未入力生徒用のCSSスタイルを追加（色: `--color-info`）。
